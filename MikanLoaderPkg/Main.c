...
EFI_STATUS GetMemoryMap(struct MemoryMap *map)
{
    ...
    return gBS->GetMemoryMap(
        &map->map_size,                       // IN ...メモリマップ書き込み用メモリ領域の大きさ / OUT...実際のメモリマップの大きさ
        (EFI_MEMORY_DESCRIPTOR *)map->buffer, // IN ...メモリマップ書き込み用メモリの先頭 / OUT...メモリマップ書き込み
        &map->map_key,                        // OUT...メモリマップを識別するための値
        &map->descriptor_size,                // OUT...メモリマップの個々の行を表すメモリディスクリプタのバイト数。このsize幅の間隔で配置される
        &map->descriptor_version              // OUT...バージョン(未使用)
    );
}
